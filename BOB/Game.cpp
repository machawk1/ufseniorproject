...
int main()
{
	...
	for(i = 0; i < numPlayers; i++)
	{
		...
		cout << endl;
		cout << "  Is Player " << i << " a Human(0) or CPU(1) Player?: ";
	...
	{
		if(type[i] == 1)
			players[i] = Player(handle[i], CPU);
		else
			players[i] = Player(handle[i], Human);
		
		players[i].setAP(numBobs*10);
	}
	...
	game.initialize(players, numPlayers, numBobs*10, map1);
	...
	while(!game.gameOver())
	{
		...
		{
			cout << game.getActivePlayer()->getPlayerType() << endl;
			
			while(game.getActivePlayer()->getPlayerType() == CPU && !game.gameOver())
			{
				cout << "Beginning AI " << game.getActivePlayer()->getHandle() << " turn" << endl;
				ai.determineAction();
				cout << "Ending AI turn" << endl;
