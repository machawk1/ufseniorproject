...
#define CLIENT_PORT  29001
#define SERVER_PORT  29000
#define CLIENT_PORT2 29002
#define CLIENT_PORT3 29003
#define CLIENT_PORT4 29004
...
void BobsClient()
{
	...
	if(bobsStruct.bobsNumber > 4)
		area = bobsStruct.bobsNumber;
...
*****************************************************************************/
	...
	while(1)
	{
		printf("Player Type  (h) Human or (c) CPU: ");
		cin >> ClientData.playerType;

		if(ClientData.playerType != 'h' && ClientData.playerType != 'c')
	...
	while (SetUpLooping)
	{
		...
		//		}
			...
			case BROADCAST_MAP:
				...
				{
						for(i = mapHeight - 1; i> mapHeight - (area+1); i--)
						{	
							for(j = mapLength - 1; j> mapLength - (area+1); j--)
							{
								occupied = false;
								for(int q = 0; q < blockedX.size(); q++)
								{
				...
				{
						for(i = mapLength - 1; i > mapLength - (area+1); i--)
						{	
							for(j = area-1; j>= 0; j--)
							{
								occupied = false;
				...
				if (ClientData.myplayerid == 3)
				{
						for(i = 0; i<area; i++)
						{	
							for(j = mapHeight - area; j<= mapHeight - 1; j++)
							{
								occupied = false;
							//	printf("blockedx[%d]: %d\nblockedy[%d]: %d\n\n",i,blockedX[i],j,blockedY[j]);
								for(int q = 0; q < blockedX.size(); q++)
