...
void TileSelection::change_Selection()
{
	...
	{	
		// Convert the mouse x,y into the tile index x,y
		tile_x = m_x;
		tile_y = m_y;
		height = m_TileManager->Get_Tile_Height(tile_x, tile_y);
		BobC* bob = m_TileManager->Get_Bob_Array();

		/* Select a bob if clicked on */
		if ( m_TileManager->Get_Occupied(tile_x, tile_y) == true )
		{
				Selected_Bob = m_TileManager->Get_Unit(tile_x, tile_y);

				// Logging the selection for testing
				Logger::log().info() << "Selected robot " << Selected_Bob->getName() << " at Tile(" << tile_x << "," << tile_y << ")";
		}	
		...
		switch ( ActionState )
		{
			...
			case PLACE:			
				...
				if ( m_TileManager->Get_Occupied(tile_x, tile_y) != true && bobCount < MAX_BOB_AMOUNT)
				{
					// Code to give the Robot mesh a unique name and place it

					char NodeName[NAME_BUFFER_SIZE];
	
					sprintf( NodeName, "BobNode[%d]", bobCount );
					
					//bob[bobCount].model = "robot.mesh";
					//bob[bobCount].mEntity = m_TileManager->Get_pSceneManager()->createEntity( EntityName, bob[bobCount].model );
					bob[bobCount].x = tile_x;
					bob[bobCount].y = tile_y;
					bob[bobCount].ID = bobCount;
					bob[bobCount].mNode = m_TileManager->Get_pSceneManager()->getRootSceneNode()->createChildSceneNode(
						NodeName , Vector3(tile_x * TILE_SIZE + 16, height , tile_y * TILE_SIZE + 16));
					bob[bobCount].mNode->attachObject( bob[bobCount].mEntity );
					bob[bobCount].mNode->setScale(0.4f, 0.4f, 0.4f);
					setIdle(bobCount);
					bob[bobCount].active = true;

					// Setting the this tile as occupied by with this Node, and setting this bob as the selected bob
					m_TileManager->Set_Map_Occupied(tile_x, tile_y, true);
					m_TileManager->Set_Map_Unit(tile_x, tile_y, bob[bobCount].mNode);
					Selected_Bob = m_TileManager->Get_Unit(tile_x, tile_y);
					...
					Logger::log().info() << "Creating and Placing robot " << NodeName << " at Tile(" << tile_x << "," << tile_y << ") at a height of: " << (int)height;					
				}
				break;
			...
			case MOVE:
				...
				if ( m_TileManager->Get_Occupied(tile_x, tile_y) != true && Selected_Bob != NULL)
				{
	
					// ------------ Set the new tile map to occupied and attached the node -----------------
					//Selected_Bob->setPosition(tile_x * TILE_SIZE + 16, height, tile_y * TILE_SIZE + 16);
					m_TileManager->Set_Map_Occupied(tile_x, tile_y, true);
					m_TileManager->Set_Map_Unit(tile_x, tile_y, Selected_Bob);

					// ------------ Set the original tile to unoccupied-------------------------------------
					bobIndex = getBobIndex(Selected_Bob);
					
					m_TileManager->Set_Map_Occupied( bob[bobIndex].x, bob[bobIndex].y, false);
					m_TileManager->Set_Map_Unit( bob[bobIndex].x, bob[bobIndex].y, NullNode);
					
					// ------------ Set the new x,y values and animate the bob object -------------------------------
					bob[bobIndex].x = tile_x;
					bob[bobIndex].y = tile_y;

					Logger::log().info() << "Before attempt to add destination vector to walklist.";
                    Walklist = m_TileManager->Get_Walklist();
					Walklist->push_back(Vector3 (tile_x * TILE_SIZE + 16, height, tile_y * TILE_SIZE + 16));
					
					setWalk(bobIndex);

					Logger::log().info() << "Moving robot " << Selected_Bob->getName() << " to Tile(" << tile_x << "," << tile_y << ")";
					Logger::log().info() << "Inserted Destination Vector (" << tile_x * TILE_SIZE + 16 << "," << height << "," << tile_y * TILE_SIZE + 16 << ") into the Walklist.";
				}
				break;
		}
...
int TileSelection::getBobIndex(SceneNode *node)
{
	std::string BobName = node->getName();
	int number, number2 = 0;

	std::string digit1 = BobName.substr(8,1);
	std::string digit2 = BobName.substr(9,1);

	std::istringstream ss( digit1 );
	ss >> number;

	if (number == 1)
	{
		std::istringstream ss( digit2 );
		ss >> number2;
...
void TileSelection::setIdle( int ID )
{
	BobC* bob = m_TileManager->Get_Bob_Array();

		mEntity = bob[ID].mEntity;
		bob[ID].mAnimationState = mEntity->getAnimationState( "Idle" );
		bob[ID].mAnimationState->setLoop( true );
		bob[ID].mAnimationState->setEnabled( true );	
}
void TileSelection::setWalk( int ID )
{
	BobC* bob = m_TileManager->Get_Bob_Array();

		mEntity = bob[ID].mEntity;
		bob[ID].mAnimationState = mEntity->getAnimationState( "Walk" );
		bob[ID].mAnimationState->setLoop( true );
		bob[ID].mAnimationState->setEnabled( true );	
}
