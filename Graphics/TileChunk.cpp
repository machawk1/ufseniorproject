...
{
	m_ChunkNode = m_TileManagerPtr->Get_pSceneManager()->getRootSceneNode()->createChildSceneNode();
	m_LandNode  = m_ChunkNode->createChildSceneNode();
	m_LandNode->setPosition(m_posX * TILE_SIZE * CHUNK_SIZE_X, 0, m_posZ * TILE_SIZE * CHUNK_SIZE_Z);
	m_WaterNode = m_ChunkNode->createChildSceneNode();
	m_WaterNode->setPosition(m_posX * TILE_SIZE * CHUNK_SIZE_X, 0, m_posZ * TILE_SIZE * CHUNK_SIZE_Z);
}
...
{
	if (m_LandNode)		m_LandNode->detachAllObjects();
	if (m_WaterNode)	m_WaterNode->detachAllObjects();
		if (quality == QUALITY_LOW)
	{
			//if (m_Land_entity_low != NULL)	m_LandNode->attachObject(m_Land_entity_low);
			//if (m_Water_entity_low != NULL)	m_WaterNode->attachObject(m_Water_entity_low);
	}
	else if (quality == QUALITY_HIGH)
	{
		if (m_Land_entity_high != NULL)
		{
			m_LandNode->attachObject(m_Land_entity_high);
		}
		if (m_Water_entity_high != NULL)	m_WaterNode->attachObject(m_Water_entity_high);
	}
}
...
void TileChunk::CreateLandLow_Buffers()
{
	...
	if (numVertices == 0)  { Create_Dummy(m_Land_subMesh_low); return; }
	VertexData* vdata = new VertexData();
	...
	for (short a = x; a < x+ CHUNK_SIZE_X; ++a)
	{
		for (short b = z; b < z +CHUNK_SIZE_Z; ++b)
		{
					...
					m_TileManagerPtr->Get_Map_Height(a, b+1) > LEVEL_WATER_TOP || m_TileManagerPtr->Get_Map_Height(a+1, b+1) > LEVEL_WATER_TOP)
			{
				...
				/////////////////////////////////////////////////////////////////////////
				/*// 1. Triangle
				pReal1[o+ 10] = 0.0;	pReal1[o+ 11] = 1.0;	
				pReal1[o+ 22] = 0.0;	pReal1[o+ 23] = 1.0;	
				pReal1[o+ 34] = 0.5;	pReal1[o+ 35] = 0.5;
				// 2. Triangle
				pReal1[o+ 46] = 0.0;	pReal1[o+ 47] = 1.0;	
				pReal1[o+ 58] = 1.0;	pReal1[o+ 59] = 1.0;
				pReal1[o+ 70] = 0.5;	pReal1[o+ 71] = 0.5;
				// 3. Triangle
				pReal1[o+ 82] = 1.0;	pReal1[o+ 83] = 1.0;	
				pReal1[o+ 94] = 1.0;	pReal1[o+ 95] = 0.0;
				pReal1[o+106] = 0.5;	pReal1[o+107] = 0.5;
				// 4. Triangle
				pReal1[o+118] = 1.0;	pReal1[o+119] = 0.0;	
				pReal1[o+130] = 0.0;	pReal1[o+131] = 0.0;
				pReal1[o+142] = 0.5;	pReal1[o+143] = 0.5; */
